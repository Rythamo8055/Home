...
The app emphasizes browser-based functionality, meaning all sounds are generated client-side, and it advises users to ensure their device is not on silent and volume is up for sound features to work correctly.`,
  },
  {
    ...
    info: `Morse Mentor: An Interactive Morse Code Learning Platform

Core Purpose:
Morse Mentor is a comprehensive, interactive web application designed to make learning and mastering Morse code an engaging and effective experience. It caters to users of all levels, from absolute beginners to those looking to enhance their speed and accuracy.

Key Features & Functionality:

    Learn Section (Rhythmic Learning):
        Visual & Auditory Learning: Users can learn individual Morse code characters (letters, numbers, punctuation) through a dedicated "Learn" section.
        Rhythm Items: Each character is presented with its visual representation (e.g., "A"), its Morse code (e.g., ".-"), and an associated mnemonic (e.g., "Alpha").
        Audio Playback: Users can play the Morse code sound for each character and also hear the mnemonic spoken aloud (if speech synthesis is supported by their browser).
        Interactive Engagement: This method helps users associate the sound and rhythm of Morse code with its corresponding character and a memorable word.

    Quiz Section (Practice & Reinforcement):
        Multiple Quiz Modes:
            Letter to Morse: Given a character, select the correct Morse code from multiple choices or type it.
            Morse to Letter: Given a Morse code (played audibly and/or displayed visually), select the correct character from multiple choices or type it.
            Type It Out: Given a character, the user "taps" out the Morse code using "dit" (.) and "dah" (-) buttons.
        Adaptive Learning: The quiz questions are generated by prioritizing characters the user has not yet mastered or has struggled with, promoting focused learning.
        Immediate Feedback: Users receive instant feedback on whether their answer is correct or incorrect, along with the correct answer if they were wrong.
        Gamified Elements:
            Lives: Users have a limited number of lives per quiz session.
            Score & Streak: Points are awarded for correct answers, with bonus points for consecutive correct answers (streaks).
            Session Limits: Quizzes consist of a set number of questions (e.g., 10 questions).

    Reference Section (Comprehensive Guide):
        Full Morse Code List: A complete list of all supported characters (letters, numbers, punctuation) along with their Morse codes and mnemonics.
        Search Functionality: Users can quickly search for characters, Morse codes, or mnemonics.
        Filtering: Tabs allow users to filter the list by category (All, Letters, Numbers, Punctuation).
        Audio Playback: Users can play the Morse code sound for any item in the reference list.

    Progress Section (Tracking & Motivation):
        Character Mastery: Tracks attempts and correct answers for each character, visually indicating mastery status (e.g., not attempted, low progress, good progress, mastered).
        Experience Points (XP) & Levels: Users earn XP for correct answers and daily bonuses, leveling up as they accumulate XP.
        Streaks: Tracks the current and longest quiz answer streaks.
        Badges/Achievements: Users can earn various badges for milestones like mastering character sets, reaching certain levels, achieving long streaks, or completing quiz sessions perfectly.
        Daily Bonus: A daily XP bonus encourages regular engagement.
        Overall Statistics: Displays overall mastery percentage, total XP, current level progress, longest streak, and earned badges.

    Settings (Customization):
        Sound Control: Enable or disable all application sounds (Morse code playback, mnemonics, UI feedback).
        Playback Speed: Adjust the speed of Morse code playback (dot duration in milliseconds).
        Pro Mode: A toggle for a faster, fixed playback speed, overriding the custom speed setting for a more challenging experience.

User Experience & Design:
    Interactive & Engaging: The app uses varied learning methods, quizzes, and gamification to keep users motivated.
    Aesthetic UI: Features a modern, clean "Aero" variant of the Material Design 3 dark theme, with translucent surfaces and smooth animations.
    Responsive Design: Adapts to various screen sizes, making it usable on desktops, tablets, and mobile devices.
    Offline Functionality: Implemented as a Progressive Web App (PWA) with a service worker, allowing core features to be used offline after the initial load.
    Accessibility: Includes ARIA attributes for better screen reader compatibility and keyboard navigation support.
    Snackbar Notifications: Provides non-intrusive feedback for actions like unlocking achievements, claiming bonuses, or errors.
